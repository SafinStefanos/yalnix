...
/*
 * switch_to_idle: save old's context via KCSSaveFunc (which does NOT
 * remap kstack PTEs -- see kernel.c), then set up idle's mappings.
 * When old is eventually re-scheduled and KCSSaveFunc returns here,
 * we remap back to current_process (which will be old again by then).
 */
static void switch_to_idle(PCB_t *old) {
    remap_kstack(idle_pcb);
    KernelContextSwitch(KCSSaveFunc, old, NULL);
    /* resumed here when old is rescheduled */
...
static int next_sync_id(void) {
    static int counter = 0;
    return ++counter;
}

static void waiter_enqueue(Waiter_t **head, PCB_t *proc){
...
static PCB_t *waiter_dequeue(Waiter_t **head){
...
static void enqueue_ready(PCB_t *proc) {
    proc->state = READY;
    proc->next  = NULL;
    if (ready_queue_head == NULL) { ready_queue_head = proc; return; }
    PCB_t *tail = ready_queue_head;
    while (tail->next != NULL) tail = tail->next;
    tail->next = proc;
}

static void block_and_switch(PCB_t *proc, UserContext *uc) {
...
int sys_acquire(PCB_t *proc, UserContext *uc, int lock_id){
    ...
    if (l == NULL) { TracePrintf(0, "sys_acquire: lock %d not found\n", lock_id); return ERROR; }
    if (l->owner_pid == proc->pid){
        TracePrintf(0, "sys_acquire: pid=%d already owns lock %d\n", proc->pid, lock_id);
        return SUCCESS;
    }
    while (l->owner_pid != -1 && l->owner_pid != proc->pid){
...
int sys_release(PCB_t *proc, int lock_id) {
    Lock_t *l = find_lock(lock_id);
    if (l == NULL) { TracePrintf(0, "sys_release: lock %d not found\n", lock_id); return ERROR; }
    if (l->owner_pid != proc->pid) { TracePrintf(0, "sys_release: pid=%d does not own lock %d\n", proc->pid, lock_id); return ERROR; }
...
int sys_cvar_signal(int cvar_id){
    ...
    if (c == NULL) { TracePrintf(0, "sys_cvar_signal: cvar %d not found\n", cvar_id); return ERROR; }
...
int sys_cvar_broadcast(int cvar_id){
    Cvar_t *c = find_cvar(cvar_id);
    if (c == NULL) { TracePrintf(0, "sys_cvar_broadcast: cvar %d not found\n", cvar_id); return ERROR; }
    PCB_t *waiter;
...
int sys_cvar_wait(PCB_t *proc, UserContext *uc, int cvar_id, int lock_id) {
    Cvar_t *c = find_cvar(cvar_id);
    if (c == NULL) { TracePrintf(0, "sys_cvar_wait: cvar %d not found\n", cvar_id); return ERROR; }
    Lock_t *l = find_lock(lock_id);
    if (l == NULL) { TracePrintf(0, "sys_cvar_wait: lock %d not found\n", lock_id); return ERROR; }
    if (l->owner_pid != proc->pid) { TracePrintf(0, "sys_cvar_wait: pid=%d does not own lock %d\n", proc->pid, lock_id); return ERROR; }

    PCB_t *next_owner = waiter_dequeue(&l->waiters);
    if (next_owner != NULL) { l->owner_pid = next_owner->pid; enqueue_ready(next_owner); }
    else l->owner_pid = -1;
...
int sys_pipe_read(PCB_t *proc, UserContext *uc, int pipe_id, void *buf, int len){
    ...
    if (p == NULL) { TracePrintf(0, "sys_pipe_read: pipe %d not found\n", pipe_id); return ERROR; }
    while (p->len == 0) {
    ...
    if (remaining > 0) memmove(p->buf, p->buf + n, remaining);
    p->len = remaining;
    TracePrintf(0, "sys_pipe_read: pid=%d read %d bytes from pipe %d\n", proc->pid, n, pipe_id);
    PCB_t *writer = waiter_dequeue(&p->writers);
    if (writer != NULL) { TracePrintf(0, "sys_pipe_read: waking writer pid=%d\n", writer->pid); enqueue_ready(writer); }
    return n;
}
...
int sys_pipe_write(PCB_t *proc, UserContext *uc, int pipe_id, void *buf, int len){
    if (buf == NULL || len <= 0) return ERROR;
    Pipe_t *p = find_pipe(pipe_id);
    if (p == NULL) { TracePrintf(0, "sys_pipe_write: pipe %d not found\n", pipe_id); return ERROR; }
    int written = 0;
    char *src = (char *)buf;
    while (written < len) {
        while (p->len >= PIPE_BUFFER_LEN) {
        ...
        int space = PIPE_BUFFER_LEN - p->len;
        ...
        if (reader != NULL) { TracePrintf(0, "sys_pipe_write: waking reader pid=%d\n", reader->pid); enqueue_ready(reader); }
    }
    return written;
}
...
int sys_reclaim(int id){
    ...
    if (lp != NULL) {
        ...
        if (lprev == NULL) lock_table = lp->next; else lprev->next = lp->next;
        free(lp);
        TracePrintf(0, "sys_reclaim: freed lock %d\n", id);
        return SUCCESS;
    }
    Cvar_t *cp = NULL, *cprev = NULL;
    for (Cvar_t *c = cvar_table; c != NULL; cprev = c, c = c->next)
        if (c->id == id) { cp = c; break; }
    if (cp != NULL) {
        PCB_t *w;
        while ((w = waiter_dequeue(&cp->waiters)) != NULL) enqueue_ready(w);
        if (cprev == NULL) cvar_table = cp->next; else cprev->next = cp->next;
        free(cp);
        TracePrintf(0, "sys_reclaim: freed cvar %d\n", id);
        return SUCCESS;
    }
    Pipe_t *pp = NULL, *pprev = NULL;
    for (Pipe_t *p = pipe_table; p != NULL; pprev = p, p = p->next)
        if (p->id == id) { pp = p; break; }
    if (pp != NULL) {
        PCB_t *w;
        while ((w = waiter_dequeue(&pp->readers)) != NULL) enqueue_ready(w);
        while ((w = waiter_dequeue(&pp->writers)) != NULL) enqueue_ready(w);
        if (pprev == NULL) pipe_table = pp->next; else pprev->next = pp->next;
        free(pp);
